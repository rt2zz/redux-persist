...
function getStoredStateV4(v4Config: V4Config) {
  return new Promise((resolve, reject) => {
    ...
    const avoidlist = v4Config.avoidlist || []
    const allowedlist = v4Config.allowedlist || false
    ...
    storage.getAllKeys((err, allKeys = []) => {
      ...
      let keysToRestore = persistKeys.filter(passAllowedlistAvoidList)
    ...
    function passAllowedlistAvoidList(key) {
      if (allowedlist && allowedlist.indexOf(key) === -1) return false
      if (avoidlist.indexOf(key) !== -1) return false
      return true
    }
